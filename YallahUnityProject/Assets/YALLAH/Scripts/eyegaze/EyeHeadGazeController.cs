...
public class EyeHeadGazeController : MonoBehaviour {
	...
	// The core logic to control eye/neck rotation
	private EyeHeadGazeLogic eyeGazeLogic;

	// The objct that is possibly found when asked to LookAt a target via string name.
...
//	public float neckRestTendencyStrenght;
	...
	// The list of the IDs of the blendshapes required by the eye gaze controller.
	...
	void Awake() {
		...
		this.eyeGazeLogic = new EyeHeadGazeLogic(
	...
	public void StopLooking() {
		this.eyeGazeLogic.eyeGazeTargetPoint = null;
	}
	...
	public void LookAtPoint(Vector3 targetPoint){
        ...
        lookedObject = null;
        this.eyeGazeLogic.LookAtPoint(targetPoint);
	...
	public void LookAtObject(string target_obj){
		lookedObject = GameObject.Find (target_obj);
		if(lookedObject != null) {
            this.eyeGazeLogic.LookAtPoint(lookedObject.transform.position);
		}
	}
	...
	void LateUpdate() {
        ...
        if (lookedObject != null)
        {
            this.LookAtObject(lookedObject.transform.name);
        }
   
        //
        // Copy the preferences from the Unity panel to teh logic.
        this.eyeGazeLogic.enableNeckRotation = this.enableNeckRotation;
        ...
        //
        // Invoke the animator
		this.eyeGazeLogic.UpdateEyesRotation (
