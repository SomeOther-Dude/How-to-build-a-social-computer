...
public class VSMRemoteConnector : MonoBehaviour
{
    ...
    private InPlaceRotationController _inPlaceRotationController;
    ...
    private CameraCinemaControl _cameraController;
    ...
    void Start()
    {
        ...
        {
            if (args[i] == "--vsm-socket-address" && args.Length > i + 1)
            {
                string sock_address = args[i + 1];
                Debug.Log("Replacing VSM socket address with " + sock_address);
        ...
        this._inPlaceRotationController = gameObject.GetComponent<InPlaceRotationController>();
        Debug.Assert(this._inPlaceRotationController != null, "Couldn't find the InPlaceRotationController in the avatar.");
        ...
        // The camera controller will be automatically serached in the maibn camera.
        this._cameraController = Camera.main.GetComponent<CameraCinemaControl>();
        if(this._cameraController == null)
        {
            Debug.LogError("Camera Cinema Controller not found. It was searched in the _main_ camera '" + Camera.main + "'");
        }
        else
        {
            Debug.Log("Camera cinema control found on Camera '" + this._cameraController.gameObject.name + "'");
        }
...
*/
    ...
    private void _handleCommand(string cmd, Dictionary<string, string> parameters)
    {
        ...
        else if (cmd == "SetCameraShot")
        {
            string shot_name = parameters["type"];
            if (this._cameraController != null)
            {
                this._cameraController.SetCurrentShot(shot_name);
            }
            else
            {
                Debug.LogError("Camera Controller not set. It was searched in the 'main' camera");
            }
